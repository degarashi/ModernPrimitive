...
- **HUD (Heads-Up Display)**: パラメータ値やスナップ情報をリアルタイムで画面上に表示。オブジェクトのスケールが不均一な場合にワールド空間の値を併せて表示可能（プリファレンス → HUD で設定、**デフォルトでは無効**）。
<img src="./doc_images/world_space_scale_hud.jpg" alt="World-Space HUD Display" width="75%" />
- **モーダル編集 (`Ctrl + Shift + C`)**: キーボードショートカット（サイズは `S`、高さは `H`、スムーズは `W` など）、数値入力、マウスホイールを使用してパラメータを調整できるモーダルモード。
...
## 推奨環境
- **Blender 4.3 以降**（Blender 5.2 以降も対応）
